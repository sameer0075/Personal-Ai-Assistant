/**
 * Every LinkedIn tool declares `userId` as OPTIONAL in its zod schema
 * (see userIdField in linkedin.tools.ts) specifically so a missing value never
 * gets rejected by zod itself with a terse, confusing validation error. Instead
 * every handler calls this first and gets a clear, actionable message pointing
 * at the actual likely cause.
 *
  ...
  if (!userId) {
    throw new Error(
      "Internal error: no authenticated user reached this tool call. This usually means the backend and " +
        "mcp-linkedin processes are out of sync (e.g. one was restarted/rebuilt without the other) - " +
