...
import { buildSupervisorGraph } from "./supervisor.agent.js";
...
/**
 * The agent is now a coordinator (supervisor) that routes each request to a
 * bespoke specialist - knowledge, email, calendar, LinkedIn, web/general - see
 * supervisor.agent.ts. Each specialist only holds the tools for its domain, and
 * every message any of them produces is appended to `collector` so this graph's
 * shared tool-trace / pending-action extraction sees them all.
 */
async function getAgent(collector: BaseMessage[]): Promise<CompiledAgent> {
  return buildSupervisorGraph(collector);
}
...
export async function* streamAssistantAgent(
  ...
  try {
    ...
    const agent = await getAgent(allMessages);
