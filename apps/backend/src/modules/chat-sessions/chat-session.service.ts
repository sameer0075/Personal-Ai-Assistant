...
export function listSessions(userId: string, workspaceId: string): Promise<ChatSession[]> {
  return chatSessionRepository.listSessions(userId, workspaceId);
}

export async function getAllMessages(sessionId: string, userId: string, workspaceId: string): Promise<ChatMessageRecord[]> {
  const session = await chatSessionRepository.getSession(sessionId, userId, workspaceId);
  if (!session) throw new Error("Chat not found");
  return chatSessionRepository.getAllMessages(sessionId);
}

export function deleteSession(sessionId: string, userId: string, workspaceId: string): Promise<void> {
  return chatSessionRepository.deleteSession(sessionId, userId, workspaceId);
}

export async function getOrCreateSession(userId: string, workspaceId: string, sessionId?: string): Promise<ChatSession> {
  if (sessionId) {
    const existing = await chatSessionRepository.getSession(sessionId, userId, workspaceId);
    if (!existing) throw new Error("Chat not found");
    return existing;
  }
  return chatSessionRepository.createSession(userId, workspaceId);
}
...
export async function editUserMessage(sessionId: string, userId: string, workspaceId: string, messageId: string): Promise<void> {
  const session = await chatSessionRepository.getSession(sessionId, userId, workspaceId);
  if (!session) throw new Error("Chat not found");
...
export function indexTurnForRecall(userId: string, workspaceId: string, sessionId: string, question: string, answer: string): void {
  ingestText({
    userId,
    workspaceId,
